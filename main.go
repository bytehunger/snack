...
func main() {
	...
	err = cp.Copy("templates/"+site.Theme+"/assets", "public/assets")

	if err != nil {
		panic("Could copy assets")
	}
}
...
func (p *Page) Render(site *Site) error {
	header, err := template.ParseFiles("templates/" + site.Theme + "/header.html.tpl")

	if err != nil {
		return err
	}

	footer, err := template.ParseFiles("templates/" + site.Theme + "/footer.html.tpl")
...
func (w *Widget) Render(f io.Writer, s *Site) error {
	...
	path := "templates/" + s.Theme + "/widgets/"
